...
class MyHashSet
  ...
  def union(set2)
    ...
    (@store.keys + set2.to_a).each { |el| new_set.insert(el) }
    new_set
  end

  def intersect(set2)
    new_set = MyHashSet.new
    set2.to_a.each { |el| new_set.insert(el) if @store.include?(el) }
    new_set
  end
  ...
  def symmetric_difference(set2)
    ...
    union.to_a - intersect.to_a
  end

  def == (object)
    if object.class == MyHashSet
      return true if symmetric_difference(object) == []
    end
    false
  end
end
