...
class String
  def caesar(shift)
    self.chars.map do |char|
      case
      when char.ord < 65 || char.ord > 123
        char = char
      when 65 < char.ord + shift && char.ord + shift <= 90
        char = (char.ord + shift).chr
      when 97 < char.ord + shift && char.ord + shift <= 122
        char = (char.ord + shift).chr
      else
...
class Hash
  def difference(other_hash)
    merge(other_hash) do |key, oldval, newval|
      has_key?(key) && other_hash.has_key?(key)
    end.reject{ |k, v| v == true }
  end
end
...
class Fixnum
  def stringify(base)
    ...
    until self / (base ** power) == 0
      result << Hex[self / (base ** power) % base]
      power += 1
    end
    result.reverse
  end
